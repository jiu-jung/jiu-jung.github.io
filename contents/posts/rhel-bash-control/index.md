...
조건식 대괄호 안에는 비교식이나 검사식이 들어간다.

사용할 때는 다음 두 가지를 주의해야 한다.
- 앞뒤 공백이 꼭 필요하다.
...
운영 자동화에서는 다음 같은 것들을 함수로 자주 분리한다.
...
이 문법은 **명령어 출력값을 변수로 받아 이후 조건문이나 반복문에서 활용할 때** 자주 사용한다.
...
## 간단한 예제: 로그 파일 점검 스크립트
---

지금까지 본 요소를 간단히 묶으면 이런 형태가 된다.
...
check_log() {
    ...
    if [ "$count" -gt 0 ]; then
        echo "[WARN] $file: $count matches found"
    else
        echo "[OK] $file: no matches"
    fi
}
