...
프로세스는 **PID, 부모-자식 관계, 실행 상태, 자원 사용 정보**등을 가지며,   
운영체제는 이를 기준으로 프로그램의 실행을 추적하고 제어한다.

이 글에서는 리눅스 프로세스가 어떤 구조로 동작하고 관리되는지 다음 순서로 정리한다.
- [어떻게 처음 시작되는가 — **PID 1**](https://jiu-jung.github.io/rhel-process/#어떻게-처음-시작되는가)
- [어떤 구조로 연결되는가 — **프로세스 트리**](https://jiu-jung.github.io/rhel-process/#어떤-구조로-연결되는가)
- [어떻게 생성되고 실행되는가 — **fork, exec**](https://jiu-jung.github.io/rhel-process/#어떻게-생성되고-실행되는가)
- [어떻게 관찰하고 제어하는가 — **ps, top, kill, nice, renice**](https://jiu-jung.github.io/rhel-process/#어떻게-관찰하고-제어하는가)
- [정리](https://jiu-jung.github.io/rhel-process/#정리)
...
부팅 직후 커널이 실행한 **첫 번째 프로세스**에서 출발한다.  
...
이러한 **PID 1 역할**을 전통적으로는 **init** 이 맡았고, 현대 리눅스에서는 대부분 **systemd** 가 맡는다.  
...
>
> **systemd**는 서비스들을 **관계 기반으로 관리하는 운영 체계**에 가깝다.
...
서로 독립적으로 존재하는 것이 아니라,  
**부모-자식 관계를 가지는 트리 구조**를 이룬다.

그래서 프로세스를 이해할 때는  
단순히 “지금 무엇이 실행 중인가”만 보는 것이 아니라,  
**누가 누구를 생성했는가**까지 함께 봐야 한다.

이 구조를 이해하면 다음과 같은 점에서 도움이 된다.

- 어떤 프로세스가 **누가 만든 것인지**를 통해 동작 흐름을 이해할 수 있다
- 장애 분석 시 **원인을 추적**하기 쉽다
...
즉, 리눅스의 프로세스는 각각 따로 존재하는 것이 아니라,  
부모-자식 관계를 따라 연결된 구조 안에서 동작한다.
...
이런 상속 구조 덕분에,  
셸에서 환경 변수를 `export` 한 뒤 명령어를 실행하면  
...
- 자식 프로세스는 이미 종료되었지만, 부모가 종료 상태를 아직 회수하지 않아 프로세스 테이블에 남아 있는 상태
- 실제로 실행 중인 것은 아니며, 부모가 `wait()` 계열 호출로 상태를 회수하면 정리된다

<br>

> orphan은 **살아 있는 자식이 부모를 잃은 상태**이고,  
> zombie는 **이미 종료된 자식의 상태 정보가 아직 회수되지 않은 상태**다.
...
부모와 자식은 같은 코드에서 시작하지만,   
반환값이 다르기 때문에 이후 서로 다른 동작으로 분기할 수 있다.
...
*(실제 구현에서는 `clone`이나 `posix_spawn` 같은 최적화가 사용될 수 있다.)*
