...
커널은 사용자 프로그램들을 하나씩 직접 실행하지 않는다. 대신 **사용자 공간의 첫 프로세스 하나를 실행**하고, 이후의 서비스와 세션 관리는 그 프로세스가 맡는다. 이 첫 프로세스가 바로 **PID 1** 이다.
...
|관리 단위|스크립트|unit|
|의존성 관리|제한적|명시적|
|로깅 통합|없음|journald 연계|
|관리 범위|서비스 실행 중심|서비스/소켓/타이머/마운트 등 통합|
...
즉 리눅스에서 새 프로그램 실행은 보통 **프로세스 생성**과 **프로그램 교체**가 이어지는 흐름으로 이루어진다.
