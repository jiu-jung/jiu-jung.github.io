...
- **표준 SQL**을 사용해서 **Amazon S3에 있는 데이터를 쉽게 분석**할 수 있는 대화형 쿼리 서비스
- **서버리스** 서비스로, 관리할 인프라가 없고 실행한 쿼리에 대해서 비용 지불.
- **자동으로 확장**되며, 쿼리를 **병렬 실행**하여 빠르게 결과를 반환
- S3 데이터를 in-place 조회하므로, **별도 적재 없이 ad hoc 쿼리**에 적합
...
- S3 데이터에 대해 **간단한 ad hoc SQL 쿼리**를 **빠르게** 실행하고 싶을 때
...
- **S3에 있는 데이터**를 **최소 변경, 최소 운영**으로 **SQL 조회** → Athena
